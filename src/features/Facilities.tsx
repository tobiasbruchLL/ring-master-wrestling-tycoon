...
export default function Facilities({
  ...
  const facilitiesByUpgradeCost = [...state.facilities].sort((a, b) => {
    const ca = getFacilityUpgradeCost(a);
    const cb = getFacilityUpgradeCost(b);
  ...
  return (
    <div className="p-8 space-y-10">
      ...
      <div className="space-y-0 border-t border-border">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1 min-w-0">
              <div className="flex items-center gap-2 text-[10px] font-bold text-accent uppercase tracking-widest">
                <ArrowUpCircle size={12} className="shrink-0" />
                <span>
                  {`Gate from $${HQ_TICKET_PRICE_START} before buzz at the backyard; stacks +$1 on every ticket (+$${curTicketBonus} → +$${nextTicketBonus})`}
                </span>
              </div>
              {atTicketUpgradeLeagueCap && nextLeagueTicketCap != null && nextLeagueTicketCap > ticketUpgradeMax && (
                <p className="text-[9px] font-bold uppercase tracking-widest text-zinc-500 pl-[20px]">
        ...
        {facilitiesByUpgradeCost.map(facility => {
          ...
          const nextLeagueCap =
            state.leagueIndex < maxLeagueIndex()
              ? facilityMaxLevel(facility, state.leagueIndex + 1)
              : null;
